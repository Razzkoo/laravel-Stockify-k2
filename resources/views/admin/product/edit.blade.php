    ...
    <form action="{{ route('admin.product.update', $product->id) }}"
        ...
        <div>
            ...
            <div class="flex items-center gap-4">
                <div class="w-40 h-32 rounded-lg border border-dashed
                    ...
                    <img id="imagePreview" class="{{ $product->image ? '' : 'hidden' }} w-full h-full object-cover" src="{{ $product->image ? asset('storage/' . $product->image) : '' }}">
                    <span id="imagePlaceholder"
                          class="text-xs text-gray-400 dark:text-gray-500 {{ $product->image ? 'hidden' : '' }}">
                        Preview
                    </span>
                </div>
            ...
            <div>
                ...
                <div class="relative">
                    ...
                    <input type="text"
                        ...
                        value="{{ number_format(old('selling_price', $product->selling_price), 0, ',', '.') }}"
        ...
        <div class="border-2 border-gray-300 dark:border-gray-600 rounded-lg p-4">
            ...
            <div id="attribute-wrapper" class="space-y-2">
                @foreach ($product->attributes as $index => $attribute)
                    <div class="flex gap-2">
                        <input type="text" name="attributes[{{ $index }}][name]" value="{{ $attribute->name }}"
                               class="bg-gray-50 border border-gray-300 text-sm rounded-lg w-full p-2.5
                                      dark:bg-gray-700 dark:border-gray-600 dark:text-white">
                        <input type="text" name="attributes[{{ $index }}][value]" value="{{ $attribute->value }}"
                               class="bg-gray-50 border border-gray-300 text-sm rounded-lg w-full p-2.5
                                      dark:bg-gray-700 dark:border-gray-600 dark:text-white">
                    </div>
                @endforeach
        ...
        <div>
            ...
            <textarea name="description" rows="3"
                      class="bg-gray-50 border border-gray-300 text-sm rounded-lg w-full p-2.5 resize-none
                             dark:bg-gray-700 dark:border-gray-600 dark:text-white">{{ old('description', $product->description) }}</textarea>
        </div>
